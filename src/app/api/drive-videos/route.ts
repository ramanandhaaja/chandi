...
// Returns list of videos from a Google Drive folder as
  ...
  try {
    ...
    if (parentFolderId && (dayParam || debugList === "folders")) {
      ...
      if (debugList === "folders") {
        console.log("[drive-videos] Subfolders under parent", parentFolderId, folders);
    ...
    // Build Drive API query for videos within the resolved folder
    const query = encodeURIComponent(
      `'${targetFolderId}' in parents and mimeType contains 'video/' and trashed = false`
    );
    ...
    const items = files.map((f) => ({
      title: f.name,
      videoUrl: `https://drive.google.com/file/d/${f.id}/preview`,
    ...
    try {
      ...
      console.log("[drive-videos] Failed to fetch caption:", captionError);
